...
> 另一种做法是只算「想要」（约会产品多用那种，把额度加在「喜欢」上）。
> 这里选按张数，因为产品最初的需求就是「每天只能刷 N 张」。
> 改口径只需改 `mock.ts` / `cloudfunctions/swipe` 里的一处判断。

- 每日 30 张，**每天 12:00 刷新**（不是 0 点 —— 午休是个天然的使用高峰）
...
- **左边右边滑都消耗额度**，且首页徽标每滑一张就少 1
  （这条是照 `deckStore → services → mock` 的真实链路跑的）
- 耗尽后不再下发卡片，且超额度的滑动**不被记录**
...
- **发布内容做了三层校验**（见 3.3a）：前端即时提示 + 云函数本地规则 +
  微信内容安全接口 `msgSecCheck` / `imgSecCheck`
- 仍需补充：用户协议 + 隐私政策；聊天的文本/图片也建议过一遍安全接口

### 11.1 发布内容校验的三层

| 层 | 位置 | 作用 |
| --- | --- | --- |
| 即时提示 | `src/utils/moderation.ts` → 发布页 | 边写边提示，不让用户填完一屏才被拒 |
| 服务端规则 | `cloudfunctions/publishItem/moderation.js` | 真正的门。客户端可以被绕过 |
| 微信接口 | `msgSecCheck` / `imgSecCheck` | 审核要求，也是挡政治/色情/辱骂的主力 |

规则分三类：**违法违禁**（审核红线）、**平台外内容**（招聘/租房/广告/虚拟商品）、
**联系方式**（手机号/微信号/二维码/站外链接）。

#### 这里真正难的是「不误拦」

漏拦只是少挡一条，误拦会让合法用户直接流失。所以规则**不能用裸关键词**：
...
- **否定语境**：命中处前两个字内出现非/不/无/没/别 就跳过（「非微商」「没有二维码」）
- **书名号**：「平台外内容」这一类如果落在《》内就跳过（《招聘管理实务》是书不是广告）。
  **违法违禁和联系方式不豁免** —— 那是红线，不能靠一个书名号绕过去

`pnpm verify:moderation` 里「不该拦」的样例比「该拦」的更多，就是为了钉住这件事。

#### 两份规则必须同步

客户端和云函数各一份（云函数独立打包，无法共享代码）。两份必然会漂移，
所以验证脚本用同一组样例跑两边并逐条比对，不一致直接失败。
已实测：删掉云函数里的任意一条规则，测试立刻报出错在哪一条。
