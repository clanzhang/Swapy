...
**可匹配卡的数量也是不变量**。种子里的 `SEED_SWIPES` 决定了谁会跟我匹配；
如果这些物品被删了、或它们的主人在架物品归零，新用户就会「怎么点喜欢都不
匹配」，表现上跟功能坏了一样。已加断言：可匹配卡 ≥ 3 张。

#### 改种子数据要同时把 `SEED_VERSION` +1

Mock 模式把数据存在本地 Storage。如果读存档时直接信任，那么改完种子数据后
**老设备上永远是旧数据** —— 而症状是「牌堆不对」「怎么点都不匹配」这类
很难归因到种子数据的问题（这个坑真堵过）。

所以存档里存了 `seedVersion`，对不上就重新播种。验证方式：往 Storage 里塞
一份没有 `seedVersion` 的旧存档，启动后应该自动变成新数据。
