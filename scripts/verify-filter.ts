...
async function main() {
  ...
  await step('未选中的标签点一下会选中', () => {
    assert.deepEqual(toggleValue([], '乐器'), ['乐器'])
    assert.deepEqual(toggleValue(['数码'], '乐器'), ['数码', '乐器'])
  })

  await step('已选中的标签再点一下会取消（这条就是报的那个 bug）', () => {
    assert.deepEqual(toggleValue(['乐器'], '乐器'), [], '点第二下必须能取消掉')
    assert.deepEqual(toggleValue(['数码', '乐器'], '乐器'), ['数码'])
    assert.deepEqual(toggleValue(['乐器', '数码'], '乐器'), ['数码'], '不受选中顺序影响')
  })

  await step('反复点偶数次会回到原样', () => {
    let list: Category[] = []
    for (let i = 0; i < 7; i += 1) list = toggleValue(list, '潮玩')
    assert.deepEqual(list, ['潮玩'], '奇数次 = 选中')

    list = toggleValue(list, '潮玩')
